                ...
                try:
                    ...
                    if file_type in ['xlsx', 'xls']:
                        ...
                        with st.spinner("Running forecast models..."):
                            ...
                            # Save the original training data for future enhancement
                            ...
                            # The previous code with mixed indentation was causing errors
                ...
                if st.button("Generate Forecasts", key="generate_forecasts_button"):
                    with st.spinner("Running forecast models..."):
                        ...
                        if use_custom_dates:
                            ...
                            # Override forecast periods with calculated value
                            forecast_periods = delta
                            ...
                            try:
                                ...
                                # Make sure we have a proper datetime index
                                try:
                                ...
                                # Fallback to no custom index if there's an error
                                st.warning(f"Could not create optimal forecast dates: {str(e)}")
                                st.session_state['custom_future_index'] = None
                        ...
                        if future_index is not None:
                            st.success(f"ðŸ“… Forecast will use dates from {future_index[0].strftime('%Y-%m-%d')} to {future_index[-1].strftime('%Y-%m-%d')} ({len(future_index)} periods)")
                            ...
                            if config.get('arima', {}).get('advanced_training', False):
                                ...
                                with st.spinner("Running Auto ARIMA with advanced training (this may take longer)..."):
                                    ...
                                    if arima_params['seasonal']:
                                        if 'seasonal_periods' in config.get('arima', {}):
                                            arima_params['m'] = config['arima']['seasonal_periods']
                                        elif 'detected_frequency' in st.session_state:
                                            # Determine m from frequency
                                            freq = st.session_state['detected_frequency']
                                            if freq.startswith('D') or freq.startswith('B'):  # Daily data
                                                arima_params['m'] = 7  # Weekly seasonality
                                            elif freq.startswith('M') or freq.startswith('MS'):  # Monthly data
                                                arima_params['m'] = 12  # Yearly seasonality
                                            elif freq.startswith('Q') or freq.startswith('QS'):  # Quarterly data
                                                arima_params['m'] = 4  # Yearly seasonality
                                            else:
                                                arima_params['m'] = 12  # Default
                                        else:
                                            arima_params['m'] = 12  # Default monthly
                                    
                                    # Generate enhanced forecast
                                    result = generate_auto_arima_forecast(
                                    ...
                                    if 'model_order' in result:
                                        st.success(f"Auto ARIMA selected order: {result['model_order']}")
                                    if 'aic' in result and result['aic'] is not None:
                                        st.info(f"Model AIC: {result['aic']:.2f}")
                                        
                                    # Add to forecasts dictionary
                                    forecasts['Auto ARIMA'] = result
                            else:
                                # Using our clean implementation module
                                from modules.demand.auto_arima_fixed_implementation import run_auto_arima_model
                                
                                # Run the Auto ARIMA model with proper error handling
                                arima_result = run_auto_arima_model(
                                ...
                                # Add the forecast to our forecasts dictionary
                                if arima_result['success']:
                                    forecasts['Auto ARIMA'] = arima_result['result']
                                elif arima_result['result'] is not None:
                                    forecasts['Auto ARIMA (Fallback)'] = arima_result['result']
                                
                        if 'LSTM' in models_to_run:
                            try:
                                # Progress indicator for LSTM (it can take longer)
